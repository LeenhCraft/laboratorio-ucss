...
class CrudCreateView extends Controller
{
            ...
            <div class="modal-dialog">
                <form id="$this->form" class="modal-content" onsubmit="return save(this,event)">
                    ...
                    <div class="modal-header">
                        <h5 class="modal-title" id="modal-title">Crear Nuevo $this->name</h5>
    ...
    private function form()
    {
        ...
        foreach (array_slice($this->estructura, 1) as $field) {
            $input .= "\t\t\t\t<div class=\"mb-3 col-lg-12 col-12\">
                \t<label class=\"form-label text-capitalize\" for=\"$field\">$field</label>
    ...
    public function js()
    {
        ...
        $generateDropdownMenu = $this->generateDropdownMenu();
        $generateDropdownOption = $this->generateDropdownOption();
    ...
    private function edit()
    {
        ...
        function fntEdit(id) {
            resetForm();
            let ajaxUrl = base_url + "$url";
            $("#modal-title").html("Actualizar $this->name " + id);
    ...
    private function openModal()
    {
        ...
        function openModal() {
            ...
            $("#modal-title").html("Crear Nuevo $this->name");
            $("#$this->id").val("");
    ...
    private function generateDropdownMenu()
    {
        ...
        $edit = 'fntEdit(${row.' . $this->id . '})';
        $delete = 'fntEdit(${row.' . $this->id . '})';
    ...
    private function generateDropdownOption()
    {
        $onclick = 'onclick="${onClickFunction}"';
        $class = 'class="${iconClass} me-2"';
        $text = '${text}';
        return <<<EOT
        function generateDropdownOption(text, iconClass, onClickFunction) {
            if (onClickFunction) {
              return `
                <a class="dropdown-item" href="#" $onclick><i $class></i>$text</a>
              `;
            } else {
              return `
                <a class="dropdown-item disabled" href="#"><i $class></i>$text</a>
              `;
            }
        }
        EOT;
