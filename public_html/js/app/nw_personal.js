...
function fntEdit(idp) {
  ...
  $.post(ajaxUrl, { id: idp }, function (data) {
    ...
    if (data.status) {
      ...
      $(".mostrarimagen").attr("src", data.data.per_foto);
    } else {
      Swal.fire({
        title: "Error",
        text: data.message,
...
function save(ths, e) {
  ...
  let ajaxUrl = base_url + "admin/personas/save";
  $.ajax({
    type: "POST",
    url: ajaxUrl,
    data: dat,
...
function update(ths, e) {
  let dat = new FormData(ths);
  divLoading.css("display", "flex");
  let ajaxUrl = base_url + "admin/personas/update";
  $.ajax({
    type: "POST",
    url: ajaxUrl,
    data: dat,
...
function resetForm(ths) {
  ...
  $(".modal-title").html("Agregar personal");
}
