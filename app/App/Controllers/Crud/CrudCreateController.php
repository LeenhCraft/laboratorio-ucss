...
class CrudCreateController extends Controller
{
    ...
    private function methodList()
    {
        ...
        $method
            ...
            ->addBody($this->indent_1 . '$arrData[$i][\'num\'] = $num;')
            ->addBody($this->indent_1 . '$arrData[$i][\'delete\'] = 0;')
            ->addBody($this->indent_1 . '$arrData[$i][\'edit\'] = 0;')
            ->addBody("")
            ...
            ->addBody($this->indent_1 . 'if ($this->permisos[\'perm_d\'] == 1) {')
            ->addBody($this->indent_2 . '$arrData[$i][\'delete\'] = 1;')
            ->addBody($this->indent_1 . '}')
            ->addBody($this->indent_1 . 'if ($this->permisos[\'perm_u\'] == 1) {')
            ->addBody($this->indent_2 . '$arrData[$i][\'edit\'] = 1;')
            ->addBody($this->indent_1 . '}')
            ->addBody('}')
    ...
    private function methodStore()
    {
        ...
        $method
            ...
            ->addBody("\$model->setId(\"" . $this->estructura[0] . "\");")
            ->addBody("")
            ->addBody("/*\n" . '$existe = $model' . "\n" . $this->indent_1 . '->where("field", "LIKE", $data[\'field\'])' . "\n" . $this->indent_1 . '->first();')
            ->addBody('if (!empty($existe)) {')
            ->addBody($this->indent_1 . '$msg = "Ya tiene un registro creado con esta información.";')
            ->addBody($this->indent_1 . 'return $this->respondWithError($response, $msg);')
            ->addBody("}\n*/")
            ->addBody('')
            ...
            ->addBody("")
            ->addBody('if (!empty($rq)) {')
            ->addBody($this->indent_1 . "\$msg = \"Registro creado correctamente.\";")
            ->addBody($this->indent_1 . "return \$this->respondWithSuccess(\$response, \$msg);")
            ->addBody('}')
            ->addBody("")
            ->addBody('$msg = "Error al intentar guardar los datos";')
            ->addBody("")
