...
function save(ths, e) {
  let men_nombre = $("#controller").val();
  let table = $("#table").val();
  let form = $(ths).serialize();
  if (men_nombre == "" || table == "") {
    Swal.fire(
      "Atención",
      "Es necesario el nombre del controllador y la tabla para continuar.",
      "warning"
    );
    return false;
  }
  divLoading.css("display", "flex");
  let ajaxUrl = base_url + "admin/crud/save";
  try {
    $.post(ajaxUrl, form, function (data) {
      if (data.status) {
        $(".alert-success").show("slow");
        $(".alert-success-body").html("");
        $(".alert-success-body").html(data.message);
        // Toast.fire({
        //   icon: "success",
        //   title: data.message,
        // });
        ...
        $(".alert-warning-body").html(data.message);
        // Toast.fire({
        //   icon: "error",
